...
public:
    ...
    */
    ...
    {
        // 当前激光帧时间戳 // extract time stamp
        timeLaserInfoStamp = msgIn->header.stamp;
        timeLaserInfoCur = msgIn->header.stamp.toSec();

        // 提取当前激光帧角点、平面点集合 // extract info and feature cloud
        cloudInfo = *msgIn;
    ...
    */
    bool saveMapService(lio_sam::save_mapRequest& req, lio_sam::save_mapResponse& res)
    {
      ...
      // 保存历史关键帧位姿 save key frame transformations
      pcl::io::savePCDFileBinary(saveMapDirectory + "/trajectory.pcd", *cloudKeyPoses3D);
      pcl::io::savePCDFileBinary(saveMapDirectory + "/transformations.pcd", *cloudKeyPoses6D);
      // 提取历史关键帧角点、平面点集合 extract global point cloud map
      ...
      if(req.resolution != 0)
      {
        ...
        // 降采样 // down-sample and save corner cloud
        downSizeFilterCorner.setInputCloud(globalCornerCloud);
        downSizeFilterCorner.setLeafSize(req.resolution, req.resolution, req.resolution);
        downSizeFilterCorner.filter(*globalCornerCloudDS);
        pcl::io::savePCDFileBinary(saveMapDirectory + "/CornerMap.pcd", *globalCornerCloudDS);
        // 降采样 // down-sample and save surf cloud
      ...
      // 保存到一起，全局关键帧特征点集合 save global point cloud map
    ...
    */
    void publishGlobalMap()
    {
        ...
        // kdtree查找最近一帧关键帧相邻的关键帧集合 // kd-tree to find near key frames to visualize
        ...
        // 降采样 // downsample near selected key frames
        ...
        // 提取局部相邻关键帧对应的特征点云 // extract visualized and downsampled key frames
        ...
        // 降采样，发布 // downsample visualized points
    ...
    */
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        // scan-to-map，调用icp匹配 // Align clouds
        ...
        // 发布当前关键帧经过闭环优化后的位姿变换之后的特征点云 // publish corrected cloud
        ...
        // 闭环优化得到的当前关键帧与闭环关键帧之间的位姿变换 // Get pose transformation
        float x, y, z, roll, pitch, yaw;
        Eigen::Affine3f correctionLidarFrame;
        correctionLidarFrame = icp.getFinalTransformation();

        // 闭环优化前当前帧位姿 // transform from world origin to wrong pose
        Eigen::Affine3f tWrong = pclPointToAffine3f(copy_cloudKeyPoses6D->points[loopKeyCur]);

        // 闭环优化后当前帧位姿 // transform from world origin to corrected pose
        ...
        // 添加闭环因子需要的数据 // Add pose constraint
        mtx.lock();
    ...
    */
    bool detectLoopClosureDistance(int *latestID, int *closestID)
    {
        ...
        // 当前帧已经添加过闭环对应关系，不再继续添加 // check loop constraint added before
        auto it = loopIndexContainer.find(loopKeyCur);
        if (it != loopIndexContainer.end())
            return false;

        // 在历史关键帧中查找与当前关键帧距离最近的关键帧集合 // find the closest history key frame
    ...
    */
    ...
    {
        // 提取key索引的关键帧前后相邻若干帧的关键帧特征点集合 // extract near keyframes
        nearKeyframes->clear();
        int cloudSize = copy_cloudKeyPoses6D->size();
    ...
    */
    ...
    {
        // 前一帧的位姿，注：这里指lidar的位姿，后面都简写成位姿 // save current transformation before any processing
        incrementalOdometryAffineFront = trans2Affine3f(transformTobeMapped);

        // 前一帧的初始化姿态角（来自原始imu数据），用于估计第一帧的位姿（旋转部分）
        static Eigen::Affine3f lastImuTransformation;
        
        // 如果关键帧集合为空，继续进行初始化 // initialization
        ...
        // 用当前帧和前一帧对应的imu里程计计算相对位姿变换，再用前一帧的位姿与相对变换，计算当前帧的位姿，存transformTobeMapped // use imu pre-integration estimation for pose guess
        ...
        // 只在第一帧调用（注意上面的return），用imu数据初始化当前帧位姿，仅初始化旋转部分 // use imu incremental estimation for pose guess (only rotation)
    ...
    */
    void extractNearby()
    {
        ...
        // kdtree的输入，全局关键帧位姿集合（历史所有关键帧集合）// extract all the nearby key poses and downsample them
        ...
        // 加入时间上相邻的一些关键帧，比如当载体在原地转圈，这些帧加进来是合理的 // also extract some latest key frames in case the robot rotates in one position
        int numPoses = cloudKeyPoses3D->size();
        for (int i = numPoses-1; i >= 0; --i)
        {
    ...
    */
    ...
    {
        // 相邻关键帧集合对应的角点、平面点，加入到局部map中；注：称之为局部map，后面进行scan-to-map匹配，所用到的map就是这里的相邻关键帧对应点云集合 // fuse the map
        ...
        for (int i = 0; i < (int)cloudToExtract->size(); ++i)
        {
            ...
            if (laserCloudMapContainer.find(thisKeyInd) != laserCloudMapContainer.end()) 
            {
                ...
                *laserCloudSurfFromMap   += laserCloudMapContainer[thisKeyInd].second;
                ...
                // 相邻关键帧对应的角点、平面点云，通过6D位姿变换到世界坐标系下 // transformed cloud not available
        ...
        // 降采样局部角点map // Downsample the surrounding corner key frames (or map)
        downSizeFilterCorner.setInputCloud(laserCloudCornerFromMap);
        downSizeFilterCorner.filter(*laserCloudCornerFromMapDS);
        laserCloudCornerFromMapDSNum = laserCloudCornerFromMapDS->size();
        // 降采样局部平面点map // Downsample the surrounding surf key frames (or map)
        downSizeFilterSurf.setInputCloud(laserCloudSurfFromMap);
        downSizeFilterSurf.filter(*laserCloudSurfFromMapDS);
        laserCloudSurfFromMapDSNum = laserCloudSurfFromMapDS->size();

        // 太大了，清空一下内存 // clear map cache if too large
    ...
    */
    ...
    {
        // 当前激光帧角点集合降采样 // Downsample cloud from current scan
    ...
    */
    ...
    {
        // 遍历当前帧角点集合，提取出与局部map匹配上了的角点 // combine corner coeffs
        for (int i = 0; i < laserCloudCornerLastDSNum; ++i){
        ...
        // 遍历当前帧平面点集合，提取出与局部map匹配上了的平面点 // combine surf coeffs
        for (int i = 0; i < laserCloudSurfLastDSNum; ++i){
        ...
        // 清空标记 // reset flag for next iteration
        std::fill(laserCloudOriCornerFlag.begin(), laserCloudOriCornerFlag.end(), false);
        std::fill(laserCloudOriSurfFlag.begin(), laserCloudOriSurfFlag.end(), false);
    }
    ...
    */
    bool LMOptimization(int iterCount)
    {
        ...
        for (int i = 0; i < laserCloudSelNum; i++) {
            ...
            // camera -> lidar ? TODO: lidar -> camera
    ...
    void transformUpdate()
    {
        if (cloudInfo.imuAvailable == true)
        {
            ...
            if (std::abs(cloudInfo.imuPitchInit) < 1.4)
            {
                ...
                // roll角求加权均值，用scan-to-map优化得到的位姿与imu原始RPY数据，进行加权平均 // slerp roll
                transformQuaternion.setRPY(transformTobeMapped[0], 0, 0);
                imuQuaternion.setRPY(cloudInfo.imuRollInit, 0, 0);
                tf::Matrix3x3(transformQuaternion.slerp(imuQuaternion, imuWeight)).getRPY(rollMid, pitchMid, yawMid);
                transformTobeMapped[0] = rollMid;

                // pitch角求加权均值，用scan-to-map优化得到的位姿与imu原始RPY数据，进行加权平均 // slerp pitch
    ...
    */
    void addGPSFactor()
    {
        ...
        // 如果没有关键帧，或者首尾关键帧距离小于5m，不添加gps因子 // wait for system initialized and settles down
        if (cloudKeyPoses3D->points.empty())
            return;
        else
        {
        ...
        // 位姿协方差很小，没必要加入GPS数据进行校正 // pose covariance small, no need to correct
        ...
        while (!gpsQueue.empty())
        {
            ...
            else
            {
                ...
                // GPS噪声协方差太大，不能用 // GPS too noisy, skip
                ...
                // (0,0,0)无效数据 // GPS not properly initialized (0,0,0)
                if (abs(gps_x) < 1e-6 && abs(gps_y) < 1e-6)
                    continue;

                // 每隔5m添加一个GPS里程计 // Add GPS every a few meters
    ...
    */
    void saveKeyFramesAndFactor()
    {
        ...
        // transformTobeMapped更新当前帧位姿 // save updated transform
        ...
        // 当前帧激光角点、平面点，降采样集合 // save all the received edge and surf points
        pcl::PointCloud<PointType>::Ptr thisCornerKeyFrame(new pcl::PointCloud<PointType>());
        pcl::PointCloud<PointType>::Ptr thisSurfKeyFrame(new pcl::PointCloud<PointType>());
        pcl::copyPointCloud(*laserCloudCornerLastDS,  *thisCornerKeyFrame);
        pcl::copyPointCloud(*laserCloudSurfLastDS,    *thisSurfKeyFrame);

        // 保存特征点降采样集合 // save key frame cloud
        cornerCloudKeyFrames.push_back(thisCornerKeyFrame);
        surfCloudKeyFrames.push_back(thisSurfKeyFrame);

        // 更新里程计轨迹 // save path for visualization
    ...
    */
    void correctPoses()
    {
        ...
        if (aLoopIsClosed == true)
        {
            // 清空局部map // clear map cache
            laserCloudMapContainer.clear();
            // 清空里程计轨迹 // clear path
            globalPath.poses.clear();
            // 更新因子图中所有变量节点的位姿，也就是所有历史关键帧的位姿 // update key poses
    ...
    */
    ...
    {
        // 发布激光里程计，odom等价map // Publish odometry for ROS (global)
        ...
        if (lastIncreOdomPubFlag == false)
        {
            ...
            if (cloudInfo.imuAvailable == true)
            {
                if (std::abs(cloudInfo.imuPitchInit) < 1.4)
                {
                    ...
                    // roll姿态角加权平均 // slerp roll
                    transformQuaternion.setRPY(roll, 0, 0);
                    imuQuaternion.setRPY(cloudInfo.imuRollInit, 0, 0);
                    tf::Matrix3x3(transformQuaternion.slerp(imuQuaternion, imuWeight)).getRPY(rollMid, pitchMid, yawMid);
                    roll = rollMid;

                    // pitch姿态角加权平均 // slerp pitch
    ...
    */
    void publishFrames()
    {
        if (cloudKeyPoses3D->points.empty())
            return;
        // 发布历史关键帧位姿集合 // publish key poses
        publishCloud(pubKeyPoses, cloudKeyPoses3D, timeLaserInfoStamp, odometryFrame);
        // 发布局部map的降采样平面点集合 // Publish surrounding key frames
        publishCloud(pubRecentKeyFrames, laserCloudSurfFromMapDS, timeLaserInfoStamp, odometryFrame);
        
        // 发布历史帧（累加的）的角点、平面点降采样集合 // publish registered key frame
        ...
        // 发布当前帧原始点云配准之后的点云 // publish registered high-res raw cloud
